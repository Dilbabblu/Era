...
export class RmanPermissionsService {
  ...
  getAllPermissionFunctions(paginationOptions: any, role: any): Promise<any[]> {

    let serviceUrl = appSettings.apiUrl + '/rmanRoleFunctionsSearch?';

    let searchString = 'roleid:' + role;
  ...
  updateRmanPermissions(rmanPermissions: any): Promise<any[]> {
    let body = JSON.stringify(rmanPermissions);
    return this.http.put<any[]>(appSettings.apiUrl + '/RMAN_ROLE_FUNCTIONS', body, httpOptions).toPromise().then(data => {
      return data;
    });
  }
  ...
  updateRmanRolePermissions(rmanPermissions: any): Promise<any[]> {
    let body = JSON.stringify(rmanPermissions);
    return this.http.put<any[]>(appSettings.apiUrl + '/updateRmanRolePermissions', body, httpOptions).toPromise().then(data => {
      return data;
    });
  }
